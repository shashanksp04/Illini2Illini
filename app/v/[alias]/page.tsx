...
async function fetchListingByAliasForMeta(alias: string): Promise<VerifiedListingDetail | PublicListingDetail | null> {
  try {
    const base = await getApiBaseUrl();
    const cookieStore = await cookies();
    const cookieHeader = cookieStore.getAll().map((c) => `${c.name}=${c.value}`).join("; ");
    const res = await fetch(`${base}/api/listings/by-alias/${encodeURIComponent(alias)}`, {
      next: { revalidate: 300 },
...
export async function generateMetadata({ params }: { params: Promise<{ alias: string }> }): Promise<Metadata> {
  ...
  const listing = await fetchListingByAliasForMeta(alias);
  const canonical = `/v/${encodeURIComponent(alias)}`;
  if (!listing) {
    return {
...
export default async function AliasListingDetailPage({ params }: { params: Promise<{ alias: string }> }) {
  const { alias } = await params;
  const base = await getApiBaseUrl();
  ...
  try {
    ...
    const res = await fetch(
      `${base}/api/listings/by-alias/${encodeURIComponent(alias)}`,
      anon
        ? { next: { revalidate: 60 } }
  ...
  const canonicalUrl = `${APP_URL}/v/${encodeURIComponent(alias)}`;
    ...
    <PageContainer>
      ...
      <div className="grid gap-8 lg:grid-cols-12">
        <main className="space-y-6 lg:col-span-8">
          ...
          {isVerifiedView && (
            <Link href={`/listings/${l.id}/report`} className="inline-flex items-center text-sm font-medium text-gray-400 transition-colors hover:text-accent">
              Report this listing
            </Link>
          )}
          ...
          {isVerifiedView && verifiedListing ? (
            <div className="lg:sticky lg:top-24">
              <SellerCard
                ...
                contactHref={`/listings/${l.id}/contact`}
                reportHref={`/listings/${l.id}/report`}
              />
            </div>
          ) : requiresLogin ? (
            <div className="rounded-2xl border border-gray-200/60 bg-white p-6 shadow-card">
              ...
              <Link href={`/login?redirect=/v/${encodeURIComponent(alias)}`} className="mt-5 inline-flex w-full items-center justify-center rounded-xl bg-accent px-4 py-3 text-sm font-semibold text-white shadow-button transition-all hover:bg-accent-hover hover:shadow-button-hover focus:outline-none focus:ring-2 focus:ring-accent/30 focus:ring-offset-2">
                Log in
              </Link>
            </div>
          ) : null}
